/*
 * ============================================================================
 * Arquivo: AlvoTest.java
 * Pacote:  com.autotarget (test)
 * ============================================================================
 *
 * DESCRIÇÃO TÉCNICA:
 *   Classe de testes unitários JUnit para a hierarquia de classes Alvo
 *   (Alvo → AlvoComum, AlvoRapido). Valida criação, polimorfismo de
 *   movimento, cálculo de distância euclidiana, gerenciamento de estado
 *   e verificação de que Alvo é uma Thread.
 *
 * TÓPICOS DA RUBRICA ATENDIDOS NESTE ARQUIVO:
 *
 *   ► Testes unitários (6.1.7):
 *     - ≥3 testes cobrindo pontos críticos + casos de borda:
 *       • testCriacaoAlvoComum — valida atributos após construção
 *       • testAlvoRapidoTemVelocidadeSuperior — velocidade efetiva 2x
 *       • testPolimorfismoMover — referência Alvo, comportamento polimórfico
 *       • testCalcularDistanciaHorizontal/Vertical/Diagonal/MesmoPonto —
 *         distância euclidiana (base para colisão)
 *       • testDesativarAlvo — toggle ativo/inativo
 *       • testAlvoEhThread — instanceof Thread (requisito 6.1.4)
 *
 *   ► Herança e polimorfismo (6.1.8):
 *     - testPolimorfismoMover() cria AlvoComum e AlvoRapido como referência
 *       Alvo (polimorfismo), chama mover() 10 vezes e verifica que ambos
 *       se moveram — demonstra polimorfismo no percorrimento da lista.
 *     - testAlvoRapidoTemVelocidadeSuperior() valida que getVelocidade()
 *       retorna valor diferente para cada subclasse (override).
 *
 * TOTAL DE TESTES: 8
 *
 * ============================================================================
